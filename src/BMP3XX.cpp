...
bool BMP3XX::begin() {
  ...
  uint8_t err = readRegister(REG_ERROR);
  if (err == 0xFF || err != 0) {
    return false;  // failed to read or sensor reports error
  }
 ...
 *
 */
void BMP3XX::configure(uint8_t press_os, uint8_t temp_os, uint8_t iir_filter,
                       uint8_t odr) {
 ...
 * Configure the sensor.
 *
 * Writes the oversampling, IIR filter and output data rate settings to the
 * sensor.
 *
 */
...
uint8_t BMP3XX::readRegister(uint8_t reg) {
  ...
  if (Wire.endTransmission(false) != 0) {
    // sensor not responding to write
    return 0xFF;
  }

  Wire.requestFrom(BMP3XX_ADDRESS, (uint8_t)1);
 ...
 *
 * Reads the calibration coefficients from the sensor and stores them in the
 * calibration struct.
 *
 */
void BMP3XX::readCoefficients() {
  uint16_t par_t1 = (uint16_t)readRegister(NVM_PAR_T1_0) |
                    ((uint16_t)readRegister(NVM_PAR_T1_1) << 8);
  int16_t par_t2 = (int16_t)readRegister(NVM_PAR_T2_0) |
                   ((int16_t)readRegister(NVM_PAR_T2_1) << 8);
  ...
  int16_t par_p9 = (int16_t)readRegister(NVM_PAR_P9_0) |
                   ((int16_t)readRegister(NVM_PAR_P9_1) << 8);
  int8_t par_p8 = (int8_t)readRegister(NVM_PAR_P8);
  int8_t par_p7 = (int8_t)readRegister(NVM_PAR_P7);
  uint16_t par_p6 = (uint16_t)readRegister(NVM_PAR_P6_0) |
                    ((uint16_t)readRegister(NVM_PAR_P6_1) << 8);
  uint16_t par_p5 = (uint16_t)readRegister(NVM_PAR_P5_0) |
                    ((uint16_t)readRegister(NVM_PAR_P5_1) << 8);
  int8_t par_p4 = (int8_t)readRegister(NVM_PAR_P4);
  int8_t par_p3 = (int8_t)readRegister(NVM_PAR_P3);
  int16_t par_p2 = (int16_t)readRegister(NVM_PAR_P2_0) |
                   ((int16_t)readRegister(NVM_PAR_P2_1) << 8);
  int16_t par_p1 = (int16_t)readRegister(NVM_PAR_P1_0) |
                   ((int16_t)readRegister(NVM_PAR_P1_1) << 8);
  ...
  coeffs.par_t3 =
      par_t3 / 281474976710656.0f;  // precomputed 2^48 = 281474976710656

  coeffs.par_p11 =
      par_p11 /
      36893488147419103232.0f;  // precomputed 2^65 = 36893488147419103232.0
  coeffs.par_p10 =
      par_p10 / 281474976710656.0f;  // precomputed 2^48 = 281474976710656.0
  coeffs.par_p9 =
      par_p9 / 281474976710656.0f;    // precomputed 2^48 = 281474976710656.0
  ...
  coeffs.par_p2 = (par_p2 - 16384) /
                  536870912.0f;  // precomputed 2^14 = 16384, 2^29 = 536870912
  coeffs.par_p1 = (par_p1 - 16384) /
                  1048576.0f;  // precomputed 2^14 = 16384, 2^20 = 1048576
}
...
double BMP3XX::compensatePressure(uint32_t uncompPressure) {
  ...
  double partialData3 = pow((double)uncompPressure, 2) *
                        (coeffs.par_p9 + coeffs.par_p10 * coeffs.t_lin);
  double partialData4 =
      partialData3 + pow((double)uncompPressure, 3) * coeffs.par_p11;
